...
func TestGetCFG(t *testing.T) {
	...
	o := getOpts()
	c := edgeos.NewConfig(
		edgeos.API("/bin/cli-shell-api"),
		edgeos.Bash("/bin/bash"),
		edgeos.Cores(runtime.NumCPU()),
		edgeos.Debug(*o.Debug),
		edgeos.Dir(o.SetDir(*o.ARCH)),
		edgeos.Ext("blacklist.conf"),
		edgeos.File(*o.File),
		edgeos.FileNameFmt("%v/%v.%v.%v"),
		edgeos.InCLI("inSession"),
		edgeos.Level("service dns forwarding"),
		edgeos.Method("GET"),
		edgeos.Nodes([]string{"domains", "hosts"}),
		edgeos.Poll(*o.Poll),
		edgeos.Prefix("address="),
		edgeos.STypes([]string{"file", "pre-configured", "url"}),
		edgeos.WCard(edgeos.Wildcard{Node: "*s", Name: "*"}),
	)
